...
void UMosesCameraComponent::UpdateCameraModes()
{
	...
	if (!DetermineCameraModeDelegate.IsBound())
	{
		...
		{
			UE_LOG(LogTemp, Warning, TEXT("[MosesCamera] DetermineCameraModeDelegate is NOT bound!"));
			bLoggedOnce_NoDelegate = true;
		}
		return;
	}

	if (TSubclassOf<UMosesCameraMode> CameraMode = DetermineCameraModeDelegate.Execute())
	{
		UE_LOG(LogTemp, Verbose, TEXT("[MosesCamera] Delegate returned CameraMode class: %s"),
	...
	else
	{
		...
		{
			UE_LOG(LogTemp, Warning, TEXT("[MosesCamera] Delegate is bound but returned NULL CameraMode class"));
